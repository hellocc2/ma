...
class Conversion{
	 ...
	 * @param array $data
	 */
	public function getConversionRate($time){
		...
		if ($rs->RecordCount ()) {
			while ( ! $rs->EOF ) {
				...
				if ($time == "week") {
					if (isset($date)) {
						$time_arry = explode(' - ',$date);
						if(strtotime($time_arry[1])-strtotime($row["time"])<0) {
							$date = $this -> get_week_format(strtotime($row["time"]));//$_SESSION ["ma_endtime"]
						}
					} else {
						$date = $this -> get_week_format(strtotime($row["time"]));//$_SESSION ["ma_endtime"]
					}
					$fulltime_arry = explode(' - ',$date);
					$fulltime = date("m-d",strtotime($fulltime_arry[0]))." - ".date("m-d",strtotime($fulltime_arry[1]));
					$page_list_array [$date]['time'] = $fulltime;
				} elseif ($time == "month") {
					if (isset($date)) {
						$time_arry = explode(' - ',$date);
						if(strtotime($time_arry[1])-strtotime($row["time"])<0) {
							$date = $this -> get_month_format(strtotime($row["time"]),$_SESSION ["ma_endtime"]);
						}
					} else {
						$date = $this -> get_month_format(strtotime($row["time"]),$_SESSION ["ma_endtime"]);
					}
					...
					$date = date($str, strtotime($row["time"]));
					$page_list_array [$date]['time'] = $date;
					$fulltime = $date;
				}
				$page_list_array [$date]['rate'] += $row['rate'];
				$page_list_array [$date]['regrate'] += $row['regrate'];
				$page_list_array [$date]['payrate'] += $row['payrate'];
				$page_list_array [$date]['visitdepth'] += $row['visitdepth'];
				$page_list_array [$date]['fulltime'] = $date;
				$rs->MoveNext ();
			}
		}
		
		if (!empty($time)) {
			$n_page_list_array = array();
			foreach ($page_list_array as $page_list_array_key => $value) {
				$time_arry = explode(' - ',$page_list_array_key);
				
				if (strtotime($time_arry[1]) > strtotime($_SESSION ["ma_endtime"])) {
					$day = (strtotime($_SESSION ["ma_endtime"]) - strtotime($time_arry[0]))/86400 + 1;
				} elseif (strtotime($time_arry[0]) < strtotime($_SESSION ["ma_starttime"])) {
					$day = (strtotime($time_arry[1]) - strtotime($_SESSION ["ma_starttime"]))/86400 + 1;
				} else {
					$day = (strtotime($time_arry[1]) - strtotime($time_arry[0]))/86400 + 1;
				}
				$n_page_list_array[$page_list_array_key]['rate'] = sprintf("%01.2f",$value['rate']/$day);
				$n_page_list_array[$page_list_array_key]['regrate'] = sprintf("%01.2f",$value['regrate']/$day);
				$n_page_list_array[$page_list_array_key]['payrate'] = sprintf("%01.2f",$value['payrate']/$day);
				$n_page_list_array[$page_list_array_key]['visitdepth'] = sprintf("%01.2f",$value['visitdepth']/$day);
				$n_page_list_array[$page_list_array_key]['time'] = $value['time'];
				$n_page_list_array[$page_list_array_key]['fulltime'] = $value['fulltime'];

			}
			$page_list_array = $n_page_list_array;
		}
	...
	// public function get_week_format($time,$last_time) {
		// $result = date("Y-m-d",strtotime(date("Y-m-d", $time) . " +1 week"));
		// 		
		// if (strtotime($last_time)-strtotime($result)<0){
		// $result = $last_time;
		// }
		// 
	...
	public function get_month_format($time,$last_time) {
		$result = date("Y-m-d",strtotime(date("Y-m-d", $time) . " +1 month"));
