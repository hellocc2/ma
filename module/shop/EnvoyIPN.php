 ...
 *
 */
class EnvoyIPN extends \Lib\common\Application {
	
	function __construct() {
		$responseRequest = $this->soaputils_autoFindSoapRequest();
		$data = $this->xml2array($responseRequest, 0);
		...
		if($response->payInConfirmationResult->statusCode == 0) {
			...
			$orderLang = strtolower(substr(trim($response->payInConfirmationResult->payment->merchantReference), 0, 5));
			$orderInfo = $shoppingProcess->GetOrderByCid(array('cr.ordersCid'=>$response->payInConfirmationResult->payment->merchantReference,'cr.lang'=>$orderLang));
			$order = $orderInfo['orderInfo']['order'];
			$shoppingProcess->updateOrder(array('cr.ordersCid' => $response->payInConfirmationResult->payment->merchantReference, 'cr.ordersPay' => 1, 'cr.ordersPayDetails' => $OrdersPayDetails,'cr.endTime'=>$paytime + (($order['viewStock']+$order['expressTime']) * 24 * 3600)));
		}
