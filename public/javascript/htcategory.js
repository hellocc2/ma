		...
		$.fn.w_stronglist = function(q) {
		    ...
		    if (q == 'val') {
		        ...
		        $.ajax({
		            ...
		            timeout: 30000,
		            type: "GET",
		            url: '/index.php?module=ajax&action=ajax&responseType=result&menu_action=' + q.key + '&cids=' + q.sele,
		            success: function(a) {
		                if (a.substr(0, 3) == 'var') {
		                    eval(a);
		                    var b;
		    ...
		    function initselevalue() {
		        $.ajax({
		            ...
		            timeout: 30000,
		            type: "GET",
		            url: '/index.php?module=ajax&action=category&responseType=result&menu_action=' + q.key + '&cids=' + r.val(),
		            success: function(a) {
		                if (a.substr(0, 3) == 'var') {
		                    eval(a);
		                    if (q.valueInitFun) {
		    ...
		    function loadnode(h, j, k, l, m) {
		        ...
		        $.ajax({
		            ...
		            timeout: 30000,
		            type: "GET",
		            url: '/index.php?module=ajax&action=category&responseType=result&act=' + m + '&cid=' + j,
		            success: function(a) {
		                if (a.substr(0, 3) == 'var') {
		                    eval(a);
		                    var b = [];
...
//		alert(jq("#language").val());
		...
		var as_url = '/index.php?module=ajax&action=category&responseType=result&menu_action=get_childrenList&_language_station='+ sitelang +'&_c_1_select_id='+data.cid+'';
		console.log(as_url);
	...
	function addtypevalues_and_ajax($el,data,cb){
		...
		if((tmp1+$el.val()+tmp1).indexOf(','+data.cid+',')==-1){
			...
			var as_url = '/index.php?module=ajax&action=ajax&responseType=result&menu_action=getProductElevate&_value='+ lang +'&_categoryId='+data.cid+'';
