...
upFirstLetter:function(string){
	...
	errormsg:function(){
			var s="errormsg";
			var d=$('<div style="position:absolute; top:0; left:0; border:solid 10px red; width:80%; height:100px;"></div>').prependTo($('body'));
			$(d).sticky();
			$(d).jso();
	},
	...
	//如果是空或null就返回''
	showStr:function(s){
		if(s==null  || $.type(s)==undefined  ||  $.type(s)==null || s=='null'){
			return '';
		}else{
			return s;
		}
	...
	parseFloat3:function(v){
		return Math.round(v*1000)/1000; 
	},
	parseFloat2:function(v){
	...
	openURLByConfirm:function(url){
		var r=confirm("是否跳到页面"+url);
		if (r==true){
			window.location.href = url;
		  }
	},
	...
	asult:function(result){
		if ($.type(result)=='string'){
			result=$.parseJSON(result);
		}
		//console.log(result);
		//if(result.success!=null &&result.msg!=null){
	...
	jpAjax:function(param){
		$.ajax({
			...
			success:function(result){
				if(param.sf){param.sf(result);}
			},
			complete:function(){
				if(param.cf){param.cf();}
			},
			failure:function(){
				if(param.ff){param.ff();}
			},
			statusCode:{
			    404: function() {
			      alert('404 error');
	...
	prompt:function(title, defaultString, onCommit, onCancel){
		var sResult=prompt(title ,defaultString);
		if(sResult!=null)
		{//确定的时候
		       onCommit(sResult);
		}
		...
		{//取消的时候
			...
			mergeArray:function(arrayA, arrayB){
				...
				if(arrayA.length<1){
					...
					//如果都不是空的，再把arrayA分别一行一行塞进arrayB去 
	             	$.each(arrayA, function(iA, itemA){
						itemA.pushFlag=true;
						
	                 	//判断是否要塞进去,//如果相等就标记为false
	                 	$.each(arrayB, function(iB, itemB){
	                     	if(itemA==itemB){
	                     		itemA.pushFlag=false;
	                         	return false;
	                     	}
	                    });
